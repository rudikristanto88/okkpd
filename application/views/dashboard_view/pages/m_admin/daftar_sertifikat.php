            ...
            <div class="body">
                    ...
                    <div class="row">
                      <div class="col-md-3">
                          <select class="form-control" style="color:black" name="category">
                            <option value="noreg" <?php if(isset($_GET['category'])){ if($_GET['category'] == 'noreg'){ echo 'selected';}} ?>>Nomor Registrasi</option>
                            <option value="nosertifikat" <?php if(isset($_GET['category'])){ if($_GET['category'] == 'nosertifikat'){ echo 'selected';}} ?>>Nomor Sertifikat</option>
                            <!-- <option value="merk" <?php if($_GET['category'] == 'merk'){ echo 'selected';} ?>>Merek</option> -->
                            <option value="usaha" <?php if(isset($_GET['category'])){ if($_GET['category'] == 'usaha'){ echo 'selected';}} ?>>Nama Usaha</option>
                            <option value="pemohon" <?php if(isset($_GET['category'])){ if($_GET['category'] == 'pemohon'){ echo 'selected';}} ?>>Nama Pemohon</option>
                          </select>

                      </div>
                      <div class="col-md-3">
                        <input type="text" class="placeholder-black" name="cari" style="color:black;background:white; padding-left:16px;" <?php if (isset($_GET['cari'])){ echo 'value="'.$_GET['cari'].'"' ;}?>>
                      </div>
              ...
              <div class="table-responsive-md">
                <table class="table table-hover" id="table-datatable" class="display">
                  ...
                  <tbody>
                    <?php $i=1; foreach ($dokumen as $form):
                      $nama_produk = "";
                      $id = "";
                      ...
                      <tr>
                        ...
                        <td>
                          <?php if($cetak == true): ?>
                            <a href="<?= base_url()?>dokumen/sertifikat/<?= $form['kode_layanan'] ?>/<?= $form['id_detail']?>" target="_blank"><button class="btn btn-success">Unduh Form</button></a>
                            <button data-toggle="modal" onClick="setData(<?= $form['id_detail'] ?>,<?= $form['uid'] ?>,'<?= $form['kode_layanan'] ?>')" data-target="#modalUpload" class='btn btn-info'>Unggah <i class="fas fa-upload"></i></button>
                          <?php else: ?>
