...
class Model_user extends MY_Model {
  ...
  public function getDataSertifikat($value){
    if($value != null){
      $query = $this->db->query("SELECT * from layanan join master_layanan on layanan.kode_layanan = master_layanan.kode_layanan where layanan.id_identitas_usaha = ".$value." and layanan.status_hasil_uji = 2 ");
      return $query->result_array();
    }else{
      return null;
    }
  ...
  public function getMasterKomoditas($kode_kelompok,$id_sektor){
    if($kode_kelompok != null){
      $query = $this->db->query("SELECT * FROM `master_komoditas` as m join komoditas_sektor s on m.id_sektor = s.id_sektor join komoditas_kelompok k on m.id_kelompok = k.id_kelompok and k.id_sektor = s.id_sektor where m.id_kelompok = '".$kode_kelompok."' and m.id_sektor = '".$id_sektor."'"
    );
      return $query->result_array();
    }else{
      return null;
